        ...
        fetch(`fetch_invoice_details.php?invoice_no=${selectedInvoiceoNo}`)
            ...
            .then(data => {
                if (Object.keys(data).length > 0) {
                    ...
                    finalTotColumn.textContent = currency + " " + data.total_total || 0;
...
function addRow(data) {
    ...
    newRow.innerHTML = `
        ...
        <td name="final_total" class="editable final_total">${data.final_total}</td>
...
document.getElementById('printButton').addEventListener('click', function (e) {
    ...
    window.location.href = `../print_invoice_memo/print_invoice_memo.html?invoice_no=${encodeURIComponent(invoice_no)}`;
});
